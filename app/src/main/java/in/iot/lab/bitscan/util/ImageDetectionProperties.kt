...
class ImageDetectionProperties(
    ...
    private val isRightEdgeDistorted: Boolean
        private get() = Math.abs(topRightPoint.y - bottomRightPoint.y) > 100

    private val isLeftEdgeDistorted: Boolean
        private get() = Math.abs(topLeftPoint.y - bottomLeftPoint.y) > 100
    ...
    private val isBottomEdgeTouchingProper: Boolean
        private get() = bottomLeftPoint.x >= previewHeight - 10 || bottomRightPoint.x >= previewHeight - 10

    private val isTopEdgeTouchingProper: Boolean
        private get() = topLeftPoint.x <= 10 || topRightPoint.x <= 10

    private val isBottomEdgeTouching: Boolean
        private get() = bottomLeftPoint.x >= previewHeight - 50 || bottomRightPoint.x >= previewHeight - 50

    private val isTopEdgeTouching: Boolean
        private get() = topLeftPoint.x <= 50 || topRightPoint.x <= 50

    private val isRightEdgeTouching: Boolean
        private get() = topRightPoint.y >= previewWidth - 50 || bottomRightPoint.y >= previewWidth - 50

    private val isLeftEdgeTouching: Boolean
        private get() = topLeftPoint.y <= 50 || bottomLeftPoint.y <= 50
